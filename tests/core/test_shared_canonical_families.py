...
from mordheim_combat_lab.catalog.loader import load_bands, runtime_bindings
from mordheim_combat_lab.core.compiler import compile_fighter
from mordheim_combat_lab.core.engine import (
    OUT, STANDING, _critical_wound_threshold, _new_state, _rescue_force_of_will, _sustain_force_of_will,
    attack_count, priority, resolve_attacks,
)
from mordheim_combat_lab.core.models import Characteristics, EffectSet, FighterBuild
...
def test_all_implemented_canonical_families_are_executable_for_every_member():
    ...
    assert document["summary"] == {"families": 70, "rules": 106, "kinds": {"compiler": 45, "mechanic": 20, "trait": 5}}
    packages = {
...
def test_unarmed_fighting_and_eshin_mastery_have_their_exact_attack_bonuses():
    ...
    monk_unarmed = compile_fighter(build(
        "battle-monks-of-cathay", "dragon-monks", main_weapon_id="weapon.natural-attacks",
    ), ROOT)
    ...
    for band_id, profile_id, collection, rule_id in cases:
        unarmed = compile_fighter(build(
            band_id, profile_id, collection=collection, main_weapon_id="weapon.natural-attacks",
...
def test_art_of_silent_death_criticals_are_always_resolved_on_to_wound():
    ...
    assert _critical_wound_threshold(cathay, natural, False) == 5
...
def test_body_slam_and_bull_charge_replace_charge_attacks(monkeypatch):
    ...
    captured = []
    from mordheim_combat_lab.core import engine
