...
from mordheim_combat_lab.core.engine import _new_state, _prepare_weapon_attack, attack_count, resolve_attacks, simulate_duel
from mordheim_combat_lab.core.models import Characteristics, DuelRequest, FighterBuild
...
def test_first_25_pending_families_moved_to_the_executable_catalogue():
    ...
    assert implemented["summary"]["families"] == 70
    assert implemented["summary"]["rules"] == 106
...
def test_anvil_head_death_blow_energy_focus_and_mark_are_executable_combat_choices():
    ...
    assert attack_count(energy, charging, True)[0] == 1
...
def test_construction_contracts_enforce_access_restrictions_and_limits():
    ...
    with pytest.raises(ValueError, match="choose exactly one Marauder tribe"):
        compile_fighter(build(
            "marauders-of-chaos", "marauder-chieftain",
            variant_ids=("tribe.norse", "tribe.hung"),
        ), ROOT)
...
def test_pirate_mercenary_access_master_of_arms_and_mutation_contracts():
    ...
    with pytest.raises(ValueError, match="at least one mutation"):
        compile_fighter(build("cult-of-the-possessed", "the-possessed"), ROOT)
