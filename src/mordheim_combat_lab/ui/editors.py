...
class FighterEditor(ttk.Frame):
    ...
    def _band_changed(self,_event=None):
        if self.is_free_selection:
            self.profile_combo.configure(values=(),state="disabled"); self.profile_name.set(""); self._stat_limits = {key: 20 for key in self.manual_characteristics}; self._render_stats(); self._configure_options(None); self.skill_checklist.set_skills(self.catalogue.skills(None)); self.summary.set("Free selection loaded · all runtime-supported options are available.")
        else:
    ...
    def _profile_changed(self,_event=None):
        ...
        self._render_stats(); self._configure_options(self.choice); self.skill_checklist.set_skills(self.catalogue.skills(self.choice)); self.summary.set(f"{profile.get('type','fighter').capitalize()} · {profile.get('cost','—')} crowns"); self._notify_change()
    ...
    def _main_weapon_changed(self,_event=None):
        ...
        if main and self.catalogue.mechanic(main).get("hands")==2: options={"Free hand":None}
    ...
    def build(self):
        values=dict(main_weapon_id=self._weapons.get(self.weapon_name.get(),"weapon.dagger"),off_hand_id=self._active_off_hands.get(self.off_hand_name.get()),armour_id=self._armours.get(self.armour_name.get(),"armour.no-armour"),defence_ids=self._selected("helmet"),main_material_id=self._materials.get(self.main_material_name.get(),"material.normal"),off_material_id=self._materials.get(self.off_material_name.get(),"material.normal"),preparation_ids=self._selected("preparation"),main_poison_id=self._poisons.get(self.main_poison_name.get()),off_poison_id=self._poisons.get(self.off_poison_name.get()),skill_ids=self.skill_checklist.selected_ids())
