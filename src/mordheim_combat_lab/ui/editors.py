"""Compact legacy-style fighter editor backed by the current KB contracts."""
from __future__ import annotations
import tkinter as tk
import re
from tkinter import ttk
from ..core.models import Characteristics, FighterBuild
from .services import CombatCatalogue, ProfileChoice
from .theme import COLORS
from .widgets import SkillChecklist
...
class FighterEditor(ttk.Frame):
    ...
    def __init__(self, parent, title: str, catalogue: CombatCatalogue, on_change=None):
        ...
        self.weapon_name = tk.StringVar(); self.off_hand_name = tk.StringVar(value="Free hand")
        ...
        self._categories = {"core", "1a", "1b", "1c", "trollheim"}; self._band_packages = {}; self._profiles = {}; self._weapons = {}; self._active_off_hands = {}
        self._equipment_vars = {}; self._equipment_options = {}
    ...
    def _build_gui(self):
        ...
        ttk.Label(self,text="SKILLS",style="Section.TLabel").pack(anchor="w",pady=(14,4)); self.skill_checklist=SkillChecklist(self,self._notify_change); self.skill_checklist.pack(fill="x")
        ttk.Label(self,text="SPECIAL RULES",style="Section.TLabel").pack(anchor="w",pady=(14,4)); self.special_rule_checklist=SkillChecklist(self,self._special_rules_changed); self.special_rule_checklist.pack(fill="x")
    ...
    def _band_changed(self,_event=None):
        if self.is_free_selection:
            self.profile_combo.configure(values=(),state="disabled"); self.profile_name.set(""); self._stat_limits = {key: 20 for key in self.manual_characteristics}; self._render_stats(); self._configure_options(None); self.skill_checklist.set_skills(self.catalogue.skills(None)); self.special_rule_checklist.set_skills(()); self.summary.set("Free selection loaded · all runtime-supported options are available.")
        else:
    ...
    def _profile_changed(self,_event=None):
        if self.is_free_selection:return
        ...
        self._render_stats(); self._configure_options(self.choice); self.skill_checklist.set_skills(self.catalogue.skills(self.choice)); self.special_rule_checklist.set_skills(self.catalogue.selectable_rules(self.choice)); self.summary.set(f"{profile.get('type','fighter').capitalize()} · {profile.get('cost','—')} crowns"); self._notify_change()
    ...
    def _render_stats(self):
        ...
        for column,key in enumerate(("WS","S","T","W","I","A")):
            self.stats_frame.columnconfigure(column,weight=1,uniform="stats")
            # A native Tk frame gives these cells a real one-pixel outline;
            # ttk.Frame borders are ignored by some Windows theme engines.
            panel=tk.Frame(self.stats_frame,background=COLORS["surface"],highlightbackground=COLORS["border_light"],highlightthickness=1,bd=0)
            panel.grid(row=0,column=column,sticky="ew",padx=(0 if column==0 else 2,2))
            tk.Label(panel,text=key,background=COLORS["surface"],foreground=COLORS["text_muted"],font=("Segoe UI Semibold",9)).pack(pady=(4,0))
            controls=tk.Frame(panel,background=COLORS["surface"]); controls.pack(fill="x",padx=4,pady=(2,5)); controls.columnconfigure(1,weight=1)
            ttk.Button(controls,text="−",style="Stat.TButton",command=lambda stat=key:self._change_stat(stat,-1)).grid(row=0,column=0,sticky="ew")
            entry=ttk.Entry(controls,textvariable=self.manual_characteristics[key],style="Stat.TEntry",width=5,justify="center"); entry.grid(row=0,column=1,sticky="ew",padx=3); entry.bind("<FocusOut>",lambda _event, stat=key:self._normalise_stat(stat))
            ttk.Button(controls,text="+",style="Stat.TButton",command=lambda stat=key:self._change_stat(stat,1)).grid(row=0,column=2,sticky="ew")
    ...
    def _main_weapon_changed(self,_event=None):
        ...
        } & set(self.special_rule_checklist.selected_ids()))
        if main and self.catalogue.mechanic(main).get("hands")==2 and not arms_master: options={"Free hand":None}
        self._active_off_hands=options; self.off_hand_combo.configure(values=tuple(options)); self.off_hand_name.set(next(iter(options),"Free hand")); self._off_hand_changed()
    def _special_rules_changed(self):
    ...
    def build(self):
        values=dict(main_weapon_id=self._weapons.get(self.weapon_name.get(),"weapon.dagger"),off_hand_id=self._active_off_hands.get(self.off_hand_name.get()),armour_id=self._armours.get(self.armour_name.get(),"armour.no-armour"),defence_ids=self._selected("helmet"),main_material_id=self._materials.get(self.main_material_name.get(),"material.normal"),off_material_id=self._materials.get(self.off_material_name.get(),"material.normal"),preparation_ids=self._selected("preparation"),main_poison_id=self._poisons.get(self.main_poison_name.get()),off_poison_id=self._poisons.get(self.off_poison_name.get()),skill_ids=self.skill_checklist.selected_ids(),special_rule_ids=self.special_rule_checklist.selected_ids())
    ...
    def load_build(self,build):
        ...
        self._set(self._weapons,self.weapon_name,build.main_weapon_id); self._main_weapon_changed(); self._set(self._active_off_hands,self.off_hand_name,build.off_hand_id); self._off_hand_changed(); self._set(self._armours,self.armour_name,build.armour_id); self._set(self._materials,self.main_material_name,build.main_material_id); self._set(self._materials,self.off_material_name,build.off_material_id)
        ...
        for skill_id,var in self.skill_checklist.variables.items():var.set(skill_id in set(build.skill_ids))
        for rule_id,var in self.special_rule_checklist.variables.items():var.set(rule_id in set(build.special_rule_ids))
        self._notify_change()
    @staticmethod
